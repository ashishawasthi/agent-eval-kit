...
The most expensive lesson of the C4 rollout was that five repos each independently discovered
their own eval metric could not fail: a redactor scored against its own output (a closed
tautology), a safety metric that re-read the product's own claims, or a golden set that planted
no PII at all, so the strictest metric was a constant 1.0 upstream of every subtlety. A metric
that cannot go red proves nothing.

This module turns that check into an assertion a repo runs in its test suite: give it the metric
function, a clean case that must score at/above the bar, and a degraded case that must score
below it. If the degraded case still passes, the metric is FALSELY GREEN and the assertion fails
with a message that says so. :func:`assert_each_can_go_red` runs it per market/segment, which is
the form the C4 lesson specifically calls for (prove it per jurisdiction, not once overall).
"""
...
def assert_each_can_go_red[T](
    ...
    *,
    ...
    The C4 lesson is that a metric must be proven able to fail PER jurisdiction, each carrying
    its OWN identifier, not once overall: a market missing from the config scores a vacuous 1.0
    that an aggregate check hides.
    """
