...
* **gate** - the promotion verdict from the shared Hrz4 AI-quality service (see
...
A repo supplies two callables (its offline ``smoke`` scorer and its ``gate`` runner) and gets a
standard CLI: ``--dataset`` and ``--mode``, aligned report output, and the fail-closed exit
codes. Exit is ``0`` iff the report passes (and, in gate mode, the authority's verdict also
passes). Lifted from ``doc-cdd-sow-agent``'s ``eval/run_eval.py`` ``main()`` (WP3), with the
CDD-specific scorers left in the repo.
"""
...
def build_parser(default_dataset: Path, description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "gate: the promotion verdict from the Hrz4 authority (requires the platform/gcp "
            "profile)."
        ),
    )
    return parser
...
def eval_main(
    *,
    ...
    gate_label: str = "Hrz4 promotion gate",
