"""Contract test for the shared Hrz4 promotion-gate client.

Guards the corrected Hrz4 client contract: a structured ``target``, top-level ``dataset_id``
equal to ``target.dataset_id``, selection by the registered bundle with no bare metric names,
response parsing from ``results[]``, a POST promotion gate, and injectable auth headers.
"""
...
from agent_eval_kit.gate_client import GateClientError, Hrz4GateClient

_BASE = "http://localhost:8084"


def _client(**kwargs: object) -> Hrz4GateClient:
    return Hrz4GateClient(_BASE, bundle="doc1-cdd-sow", model="gemini-3.5-flash", **kwargs)
...
def test_plaintext_non_loopback_base_url_is_rejected():
    with pytest.raises(ValueError, match="https://"):
        Hrz4GateClient("http://hrz.example", bundle="b", model="m")


def test_https_and_loopback_base_urls_are_accepted():
    assert Hrz4GateClient("https://hrz.example/", bundle="b", model="m")
    assert Hrz4GateClient("http://127.0.0.1:8084", bundle="b", model="m")
